<template>
  <v-data-table
    ...
    fixed-header
    dense
    @click:row="(i) => $router.push({name: 'Submission', params: { id: i.fileId } })"
  >
      ...
      <v-tooltip top>
        <template #activator="{ on, attrs }">
          <v-btn
            ...
            v-bind="attrs"
            v-on="on"
            :to="{name: 'Cluster', params: { id: item.clusterIndex } }"
    ...
    <template #item.actions="{ item }">
      <v-btn class="ml-2" color="primary" text small :to="{ name: 'Pair', params: { id: item.id } }" @click.stop="">
        Compare

...
enum RangesTupleType {
  ...
  // The upper bound of the actual range lies outside of the expected range
  // while the lower bound lies within.
  ActualShiftUpFromExpected = 2,
  // The lower bound of the actual range lies outside of the expected range
  // while the lower bound lies outside.
  ActualShiftDownFromExpected = 3,
}

/**
 * A class that holds the results of a benchmark.
 * Note: the results might be confusing at times because for example the total
 * amount of missed lines might be bigger then the range you gave. This is
 * because it counts the missed lines in *both* files. This is so because the
 * ranges are not guaranteed to be of equal length.
 */
export interface BenchmarkResults {
  // A line appears in both the expected and actual ranges
  matchedLines: number;
  // When a line is expected but not seen in the actual ranges.
  missedLines: number;
  // When a line appears in the actual ranges but is not expected.
  falseLines: number;
  // When a rangesTuple generated by the matcher is deemed invalid.
  falseMatches: number;
  // The total amount of lines in such a rangesTuple
  falseMatchingLines: number;

  // The rangesTuples in the actual list that had a match.
  matchingRangesTuples: RangesTuple[];
  // The rangesTuples in the actual list that had no matches.
  falseRangesTuples: RangesTuple[];
  // The expected ranges.
...
/**
 * Get the lineState with the hightest precedence. Will throw an error when a
 * Miss and FalseHit are compared.
 * @param lineState1 The first line state.
 * @param lineState2 The second line state.
 */
function getHigherPrecedence(
...
export class BenchmarkMatcher {
  ...
  public static toRangesTuple(
    [[from1, to1], [from2, to2]]: NumericRangesTuple
  ): RangesTuple {
  ...
  /**
   * Calculates if the ratio of the amount of lines within the expected range
   * and the amount outside is small enough.
   *
   * @param rt1 The first rangesTuple.
   * @param tr2 The second rangesTuple.
   */
  public acceptableRatio = 0.5;
  private readonly dataStructure: RangesTuple[][];

  /* eslint-disable no-invalid-this */
  private readonly map:
    Array<Array<boolean | ((rt1: RangesTuple, rt2: RangesTuple) => boolean)>> =
    [
      [ true,
        false,
        (_, rt) => this.isAcceptableRatio(rt),
        (_, rt) => this.isAcceptableRatio(rt)
      ],
      [ false,
        true,
  ...
  // TODO better naming
  private readonly file1LinesStates: LineState[] = [];
  private readonly file2LinesStates: LineState[] = [];
  ...
  /**
   * Calculates the statistics returned by the benchmark. See
   * [[BenchmarkResults]] for more information.
  ...
  public toBePresentInMatch(): void {
    ...
    for (const [, lineState]
      of this.file1LinesStates.concat(this.file2LinesStates).entries()) {
  ...
  public expect(expected: NumericRangesTuple[]): this {
    ...
    // Registers all the rangesTuple based on the line from the first range in
    // the rangesTuple. This is done so that we don't have to do a double for
    // loop over all the ranges but only the relevant ones.
    for (const rangesTuple of this.expected.values()) {
      for (let index = rangesTuple[0].from;
        index <= rangesTuple[0].to;
        index += 1) {
  ...
  private isAcceptableRatio([expected, actual]: RangesTuple): boolean {
    if (expected.from <= actual.from && actual.from <= expected.to) {
      return (expected.to - actual.from) / (expected.to - actual.to)
        >= this.acceptableRatio;
    } else {
      return (actual.to - expected.from) / (expected.from - actual.from)
        >= this.acceptableRatio;
    }
  }
  ...
  private isAcceptedCombination(
    ...
    const returnValue:
      boolean | ((rt1: RangesTuple, rt2: RangesTuple) => boolean) =
      this.map[type1][type2];
   ...
   * See [[RangesTupleType]] for more information about what the types mean.
   *
   ...
   * @param firstFile A boolean representing wether this is for the matched
   * file or the matchingfile.
   */
  private applyLineStates(
    expected: Range,
    actual: Range,
    firstFile: boolean
  ): void {

    const fileStates: LineState[] =
      firstFile ? this.file1LinesStates : this.file2LinesStates;
    ...
    for (
      ...
      fileStates[lineNumber] =
        getHigherPrecedence(fileStates[lineNumber], lineState);
    }
  }

  /**
   *
   * @param range A range from the actual rangesTuple you want all the relevant
   * expected ranges of.
   */

...
export class HTMLSummaryFormatter extends HTMLFormatter<RangesTuple[]> {
  public makeBody(jsonSummary: string): string {
    const jsonData: JSONSummaryFormat = JSON.parse(
      jsonSummary,
    ...
    for (let jsonGroupIndex = 0;
      jsonGroupIndex < jsonData.results.length;
      jsonGroupIndex += 1) {
      ...
      for (const [matchedFile, matchingFile, rangesTupleObj] of group) {
        comparisonPages.push(
          this.toComparePage(matchedFile, matchingFile, rangesTupleObj)
        );
      }
    }
    return (
  ...
  public toCompareView(
    ...
    for (const [index, [leftRange, rightRange]]
      of matchingRangesTuples.entries()) {

      const colourRotation: string = HTMLSummaryFormatter.getColourRotation(
        index,
        matchingRangesTuples.length
      );
      const id = HTMLSummaryFormatter.makeId(matchedFile, matchingFile, index);
      rightMarkedAreas.push(
        HTMLSummaryFormatter.rangeToMarkingDiv(leftRange, colourRotation, id)
      );
      leftMarkedAreas.push(
        HTMLSummaryFormatter.rangeToMarkingDiv(rightRange, colourRotation, id)
      );
  ...
  public toComparePage(
    ...
    const comparePage: string = this.toCompareView(
      matchedFile,
      matchingFile,
  ...
  private makeGroupsEntry(group: Match[], index: number): string {
    const tableRows: string[] = group.map(

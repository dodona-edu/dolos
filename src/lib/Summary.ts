...
export class Summary {
  ...
  /**
   * calculates the score, currently just returns the amount of lines in the range. A possible alternative is counting
   * the amount of k-mers.
  ...
  constructor(matches: Map<string, Matches<number>>) {
    ...
    this.results.forEach((value, key) => {
      ...
      const tempMap = new Map(
        [...value.entries()].sort(
          (val1, val2) => this.getScoreForArray(val2[1]) - this.getScoreForArray(val1[1]),
        ),
      );
    ...
    this.results = new Map(
      [...this.results.entries()].sort(
        (val1, val2) => this.getScoreForSubMap(val2[1]) - this.getScoreForSubMap(val1[1]),
      ),
    );
  }
  ...
  private getScoreForArray(arr: Array<[Range, Range]>): number {
    return arr.map(rangeTuple => Summary.getScore(rangeTuple[0])).reduce((acc, prev) => acc + prev);
  }

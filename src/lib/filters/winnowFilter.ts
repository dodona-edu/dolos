...
export class WinnowFilter extends HashFilter {
  ...
  /**
   * Generates a Winnow object with given window size and k-mer size.
   * The winnowing algorithm  will reduce the number of hash values returned by
   * the hash function. It will at least return 1 hash for every window (i.e.
   * for every windowSize characters).
   *
    ...
    for await (const byte of HashFilter.readBytes(stream)) {
      ...
      if (minPos === bufferPos) {
        ...
        yield {
          hash: buffer[minPos],
          location: filePos +
            ((minPos - bufferPos - this.windowSize) % this.windowSize),
        };
      } else {

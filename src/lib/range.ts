export class Range {
   ...
   * @param gapSize The maximum gap size you want to allow. For example if the
   * gap size is 0 then 2 cannot extend [4,5]. If you take a gap size of 1
   * however then the range can be extended and would become [2,5].
   */
  ...
  /**
   * Extends the range with the given range in place. The given range will not
   * be changed.
   ...
   * @param range The range you want to test if can be used to extend the
   * current range with.
   * @param gapSize The maximum gap size you want to allow. For example if the
   * gap size is 0 then [1, 3] and [5,6] will not be joined. If you take a gap
   * size of 1 however then the ranges can be combined into [1,6].
  ...
  /**
   * Tests if the two ranges overlap.

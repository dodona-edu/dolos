...
function indent(helpText: string): string {
  const lines: string[] = [];
...
program
  .option(
    "-l, --language <language>",
    indent("Programming language used in the submitted files."),
    "javascript",
  )
  .option(
    "-b, --base <base>",
  ...
  .option(
    ...
    indent("Specifies what format the output should be, current options are: terminal/console, json, html."),
    "terminal",
  )
  .option(

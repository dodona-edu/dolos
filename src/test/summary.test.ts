...
function containsRangesTuple(
  ...
  t.true(
    values.some(tup => tup[0].equals(expected[0]) && tup[1].equals(expected[1]))
  );
}
...
test(
  "simple match to ranges where first element stays constant and has a gap",
  t => {
...
test("test extending related functions rangesTuples", t => {
  ...
  t.true(
    summary1.canExtendRangesTupleWithRangesTuple(rangesTuple1, rangesTuple2)
  );
  t.false(
    summary1.canExtendRangesTupleWithRangesTuple(rangesTuple1, rangesTuple3)
  );
  t.true(
    summary2.canExtendRangesTupleWithRangesTuple(rangesTuple1, rangesTuple3)
  );
...
test("concatenate ranges", t => {
  ...
  // The second array will have a different order then the first.
  shuffle(shuffledArray1);
  shuffle(shuffledArray2);
  ...
  containsRangesTuple(
    t,
    concatenatedRanges1,
    [new Range(0, 30), new Range(100, 120)]
  );
